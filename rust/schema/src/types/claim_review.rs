...
/// * COMMENT: A fact-checking review of claims made (or reported) in some creative work (referenced via itemReviewed). * EXTEND FROM: https://schema.org/Review
...
pub struct ClaimReview {
    ...
    #[serde(flatten)]
    pub options: Box<ClaimReviewOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct ClaimReviewOptions {
    ...
    pub claim_reviewed: Option<ClaimReview>,
}
