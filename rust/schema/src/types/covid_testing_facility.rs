...
use super::available_service::availableService;
...
use super::health_plan_network_id::healthPlanNetworkId;
...
/// * MOD OF: https://pending.schema.org * COMMENT: A CovidTestingFacility is a <a class="localLink" href="/MedicalClinic">MedicalClinic</a> where testing for the COVID-19 Coronavirus       disease is available. If the facility is being made available from an established <a class="localLink" href="/Pharmacy">Pharmacy</a>, <a class="localLink" href="/Hotel">Hotel</a>, or other       non-medical organization, multiple types can be listed. This makes it easier to re-use existing schema.org information       about that place, e.g. contact info, address, opening hours. Note that in an emergency, such information may not always be reliable. * EXTEND FROM: https://schema.org/MedicalClinic
...
pub struct CovidTestingFacility {
    ...
    #[serde(flatten)]
    pub options: Box<CovidTestingFacilityOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct CovidTestingFacilityOptions {
    ...
    pub health_plan_network_id: Option<healthPlanNetworkId>,

    /// Whether the provider is accepting new patients.
    pub is_accepting_new_patients: Option<MedicalOrganization>,

    /// A medical specialty of the provider.
    pub medical_specialty: Option<medicalSpecialty>,

    /// A medical service available from this provider.
    pub available_service: Option<availableService>,
}

impl CovidTestingFacility {
    pub fn new() -> Self {
        Self {
            ..Default::default()
        }
