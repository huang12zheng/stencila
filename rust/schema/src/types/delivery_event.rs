...
/// * COMMENT: An event involving the delivery of an item. * EXTEND FROM: https://schema.org/Event
...
pub struct DeliveryEvent {
    ...
    #[serde(flatten)]
    pub options: Box<DeliveryEventOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct DeliveryEventOptions {
    ...
    pub access_code: Option<DeliveryEvent>,

    /// When the item is available for pickup from the store, locker, etc.
    pub available_from: Option<DeliveryEvent>,

    /// After this date, the item will no longer be available for pickup.
    pub available_through: Option<DeliveryEvent>,
