...
/// * COMMENT: A simple system that adds up the number of risk factors to yield a score that is associated with prognosis, e.g. CHAD score, TIMI risk score. * EXTEND FROM: https://schema.org/MedicalRiskEstimator
...
pub struct MedicalRiskScore {
    ...
    #[serde(flatten)]
    pub options: Box<MedicalRiskScoreOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct MedicalRiskScoreOptions {
    ...
    pub algorithm: Option<Text>,
}

impl MedicalRiskScore {
    pub fn new() -> Self {
        Self {
            ..Default::default()
        }
