...
/// * COMMENT: A PublicationEvent corresponds indifferently to the event of publication for a CreativeWork of any type, e.g. a broadcast event, an on-demand event, a book/journal publication via a variety of delivery media. * EXTEND FROM: https://schema.org/Event * LOOK ALSO: https://schema.org/BroadcastEvent, https://schema.org/OnDemandEvent
...
pub struct PublicationEvent {
    ...
    #[serde(flatten)]
    pub options: Box<PublicationEventOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct PublicationEventOptions {
