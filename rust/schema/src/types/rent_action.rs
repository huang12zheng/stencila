...
/// * COMMENT: The act of giving money in return for temporary use, but not ownership, of an object such as a vehicle or property. For example, an agent rents a property from a landlord in exchange for a periodic payment. * EXTEND FROM: https://schema.org/TradeAction
...
pub struct RentAction {
    ...
    #[serde(flatten)]
    pub options: Box<RentActionOptions>,
}

#[skip_serializing_none]
#[derive(Debug, Defaults, Clone, PartialEq, Serialize, Deserialize, Strip, Read, Write, ToHtml)]
#[serde(rename_all = "camelCase", crate = "common::serde")]
pub struct RentActionOptions {
    ...
    pub real_estate_agent: Option<RealEstateAgent>,
}
